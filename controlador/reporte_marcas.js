...
function reporte(req, res){
    ...
    modeloMarca.get_marca_estatus(estatus, (error, marcas) => { // si se pudo obtener las marcas
            if (error) { // si hubo error
                Utilidad.printError(res, {msg: `Error no se pudieron obtener las marcas: ${error}`, tipo: 0});

            }else{
                var listar = [];
                listar[0] = [{fontSize:8,borderColor: ['#cdcfd1', '#cdcfd1', '#cdcfd1', '#cdcfd1'],text: 'Código', style: 'tableHeader' }, {fontSize:8,borderColor: ['#cdcfd1', '#cdcfd1', '#cdcfd1', '#cdcfd1'],text: 'Nombre', style: 'tableHeader' }, {fontSize:8,borderColor: ['#cdcfd1', '#cdcfd1', '#cdcfd1', '#cdcfd1'],text: 'Descripción', style: 'tableHeader' }];

                for(var i = 0; i < marcas.length; i++){
                    listar[i+1] = [{fontSize:8,border: [false, false, false, false],text:marcas[i].id},{fontSize:8,border: [false, false, false, false],text:marcas[i].nombre},{fontSize:8,border: [false, false, false, false],text:marcas[i].descripcion}]
                }
                var docDefinition = {
                    ...
                    content: [{
                        ...
                        },
                        {
                            columns:[
                                {
                                    text: 'Reporte Marcas:', fontSize: 18, margin: [0, 30, 0, 10]
                                },
                                {
